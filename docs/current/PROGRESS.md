...
- 후속 해결: 사용자 UI 개선으로 탐색기 선택 행과 활성 탭이 `--color-bg`를 사용하게 되어, 이전 gray 테마의 3.78/3.04 경고는 화면에서 사용되지 않는 `--color-selected`를 계산한 값임을 확인하고 실제 선택 배경 기준으로 시험을 바로잡았다.
- 검증: 세 테마에서 본문 15.80/9.55/16.58, 보조 10.19/7.69/11.59, 실제 선택 항목 15.80·10.19/9.55·7.69/16.58·11.59, 구분선 3.03/4.90/4.27로 기준을 모두 충족한다. 손잡이를 고치기 전에는 v0.1 대비 2건이 어긋났고 고친 뒤 0건임을 같은 시험으로 확인했다.
...
| NFR-3 | 명도 대비 | 충족 | `test_appearance_parity.js` — 실제 선택 배경(`--color-bg`) 기준으로 세 테마의 본문·보조·선택 항목·주요 구분선 대비 통과 |
...
### 계획 외 개선 — 사용자 요청 UI 개선 결과

- 선택 표시: 탐색기 선택 행과 각 스플릿의 활성 탭은 콘텐츠 배경색을 쓰고, 현재 조작 중인 영역에만 1px 사각 테두리를 표시한다.
- 대비 판정: 사용되지 않는 선택색 토큰 대신 실제 선택 배경을 검사하도록 고쳐 white·gray·dark 테마가 모두 기준을 충족한다.
- 중복 탭 이동: 반대편에 같은 대상이 있으면 드래그·F6 이동을 차단하고 File > Move Tab을 비활성화한다.
- 활성 탭 표시: 모든 스플릿의 활성 탭은 콘텐츠 배경색으로 식별하고 선택된 스플릿에만 사각 포커스 테두리를 표시한다.
- 탭 테두리: 활성 탭의 상시 테두리와 탭 바 하단선을 제거해 배경색과 포커스 테두리만 남겼다.
- About 표시: Help > About 선택 시 버전 알림이 화면 아래 중앙에 3초간 표시되도록 복구했다.
- 메뉴 팝업: File·View·Help 팝업을 모두 1px 위로 올려 메뉴 바 경계와 겹치는 이중선을 제거했다.
- 스플릿 아이콘: 둥근 외곽과 흐릿한 중앙선을 각진 직사각형과 픽셀 정렬된 중앙선 하나로 바꿨다.
- 경계 배경: 탐색기/탭 및 탭/탭 조절 영역의 제목 줄 부분을 탭 바와 같은 배경색으로 연결했다.
- 탭 시작점: 탭 바 왼쪽 여백을 제거해 첫 탭이 영역 왼쪽 끝에서 시작하도록 했다.
- 탭 높이: 탭 레이블 높이를 탐색기 선택 행과 같은 22px로 맞췄다.
- 탐색기 경계: 탐색기와 탭 사이의 조작 폭은 유지하고 보이는 구분선만 옅은 1px로 줄였다.
- 스플릿 경계: 탭 스플릿의 조작 폭은 유지하고 보이는 구분선만 옅은 1px로 줄였다.
- 영역 순환: `Tab`/`Shift+Tab`으로 탐색기와 표시 중인 탭 스플릿을 양방향 순환하고 재렌더링 후에도 포커스를 유지한다.
- 검증: 관련 Node 검사 13종과 white·gray·dark Electron 선택·포커스·메뉴 상태 검사를 통과했다.
...
### 계획 외 개선 — README.md에 프로젝트 결과와 활용 방법 추가

- 요청: README.md에 현재 프로젝트의 결과와 활용 방법을 추가할 것.
- 조치:
  1. `README.md`에 `## 프로젝트 결과 (v0.1)` 절을 추가하여 리소스 독립 모듈 껍데기, 파일/폴더 프리셋, 대칭 호스트 계층, 런타임 아이콘 테마 엔진, 24개 테스트 스위트 등 v0.1 산출물 핵심을 명시했다.
  2. `## 어떻게 쓰는가 (활용 방법)` 절을 추가하여 저장소 복제 → 호스트 선택 → 프리셋 지정 → 5개 교체 슬롯 구현 → 실행으로 이어지는 5단계 개발 흐름을 기술했다.
